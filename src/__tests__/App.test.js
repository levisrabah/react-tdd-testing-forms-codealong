import '@testing-library/jest-dom'; // for custom jest matchers like toBeChecked, toHaveDisplayValue, toBeInTheDocument
...
test("size select element initially displays 'Small'", () => {
  ...
  expect(selectSize).toHaveValue("small");
});
...
test("select Size dropdown displays the user's selected value", () => {
  ...
  expect(selectSize).toHaveValue("medium");

  userEvent.selectOptions(selectSize, "large");

  expect(selectSize).toHaveValue("large");
});
